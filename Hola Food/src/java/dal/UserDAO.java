...
public class UserDAO {
    ...
    public User check(String username, String password) {
        String sql = "SELECT [id]\n"
                ...
                + "      ,[address]\n"
                + "      ,[password]\n"
                + "  FROM [dbo].[UserHE171956] where name = ? and password = ?";
    ...
    public User checkUserEmail(String username, String email) {
        String sql = "SELECT [id]\n"
                ...
                + "      ,[address]\n"
                + "      ,[password]\n"
                + "  FROM [dbo].[UserHE171956] where name = ? and email = ?";
    ...
    public User getCustomerBuyMost() {
        String sql = "select top(1) count(cid) as'NumOfOrds'\n"
                ...
                + "      ,[phonenum]\n"
                + "      ,[address]\n"
                + "  from [OrderHE171956] o join [UserHE171956] u\n"
                + "  on u.id = o.cid\n"
    ...
    public void change(User a) {
        String sql = "UPDATE [dbo].[UserHE171956]\n"
                + "   SET [password] = ?\n"
                + " WHERE [name]=?";
        try {
            PreparedStatement st = con.prepareStatement(sql);
    ...
    public boolean existedUs(String username) {
        String sql = "SELECT [name]\n"
                ...
                + "      ,[phonenum]\n"
                + "      ,[password]\n"
                + "  FROM [dbo].[UserHE171956] where name=?";
        try {
            PreparedStatement st = con.prepareStatement(sql);
            st.setString(1, username);
            ResultSet rs = st.executeQuery();
    ...
    public boolean existedEmail(String email) {
        String sql = "SELECT [name]\n"
                ...
                + "      ,[phonenum]\n"
                + "      ,[password]\n"
                + "  FROM [dbo].[UserHE171956] where email=?";
        try {
            PreparedStatement st = con.prepareStatement(sql);
    ...
    public boolean existedPhoneNum(String phonenum) {
        String sql = "SELECT [name]\n"
                ...
                + "      ,[phonenum]\n"
                + "      ,[password]\n"
                + "  FROM [dbo].[UserHE171956] where phonenum=?";
        try {
            PreparedStatement st = con.prepareStatement(sql);
    ...
    public void register(User a) {
        String sql = "INSERT INTO [dbo].[UserHE171956]\n"
                + "           ([name]\n"
                + "           ,[password]\n"
                + "           ,[fullname]\n"
                + "           ,[email]\n"
