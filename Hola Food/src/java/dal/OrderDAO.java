...
public class OrderDAO {
    ...
    public List<OrderLine> getOrderLineByUser(int id) {
        String sql = "select o.id, p.name, ol.quantity, ol.price, ol.quantity*ol.price,"
                + " o.date from OrderHE171956 o join OrderLineHE171956 ol on o.id = ol.oid"
                + " join ProductHE171956 p on ol.pid = p.ID where o.cid = ? order by date desc";
    ...
    public List<OrderLine> getOrderLineByUserPaging(int id, int index) {
        String sql = "select o.id, p.name, ol.quantity, ol.price, ol.quantity*ol.price,"
                + " o.date from OrderHE171956 o join OrderLineHE171956 ol on o.id = ol.oid"
                + " join ProductHE171956 p on ol.pid = p.ID where o.cid = ? order by o.id desc";
    ...
    public void addOrder(User u, Cart cart) {
        ...
        try {
            String sql = "insert into [OrderHE171956] values (?,?,?)";
            PreparedStatement ps = con.prepareStatement(sql);
            ps.setString(1, date);
            ps.setInt(2, u.getId());
            ...
            String sql1 = "select top 1 id from [OrderHE171956] order by id desc";
            PreparedStatement ps1 = con.prepareStatement(sql1);
            ResultSet rs = ps1.executeQuery();
            if (rs.next()) {
                int oid = rs.getInt(1);
                for (Item i : cart.getItems()) {
                    String sql2 = "insert into [OrderLineHE171956] values (?,?,?,?)";
...
//        double sum = 0;
        try {
            String sql = "SELECT date\n"
                    ...
                    + "      ,p.[price]\n"
                    + "	  , totalmoney\n"
                    + "	FROM [dbo].[OrderHE171956] o join OrderLineHE171956 ol\n"
                    + "	on o.id = ol.oid\n"
                    + "	join ProductHE171956 p\n"
                    + "	on ol.pid= p.ID\n"
                    + "\n"
...
//        double sum = 0;
    ...
    public List<OrderTag> getAllOrderTag() {
        String sql = "select o.id, u.id, u.fullname, u.phonenum, o.date, o.totalmoney from OrderHE171956 o "
                + "join UserHE171956 u on o.cid = u.id where totalmoney > 0 order by date desc";
    ...
    public List<OrderLine> getOrderLineByOId(int oId){
        String sql = "select ol.oid, p.id, p.name, ol.quantity, ol.price, ol.price*ol.quantity from OrderLineHE171956 ol "
                + "join ProductHE171956 p on p.ID = ol.pid where ol.oid = ?";
